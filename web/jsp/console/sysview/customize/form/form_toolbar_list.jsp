	...
	<div class="box">		
		<div class="tabContent">
			...
			<div class="innerBox" id="tab1_content">			  
				<div class="innerBox1">							
					<div class="btnt">
						<button class="btn-able" onclick="javascript:doWith_f(submitF,'saveFormToolbarList.action','您确定要保存吗?');">保存</button>&nbsp;&nbsp;
								...
								<tbody>
								  <s:iterator id="toolbar" name="toolbars" status="st">   								      
							        ...
							        >			
							            ...
							            <input id="id" name="id" type="radio" value="<s:write name="toolbar" property="key"/>" size="16" class="text" onclick="javascript:loadToolbarBase('<%=contextPath%>/jsp/console/getFormToolbarBase.action?resourceName=<s:write property="resourceName"/>&pageKey=<s:write property="pageKey"/>&pageCode=<s:write property="pageCode"/>&toolbarKey=<s:write name="toolbar" property="key"/>');"/>
							            </td> 				           
										<td><input id="code" name="code"  type="text" value="<s:property name="toolbar" value="code"/>" size="16" class="text"/></td>	
										<td><input id="name" name="name"  type="text" value="<s:property name="toolbar" value="name"/>" size="16" class="text"/></td>	
										<td>
										<select name="freshOpener">
								   			<option value="">请选择</option>
								   			<option value="true" <%if(ValueStack.get("toolbar","freshOpener").equals("true")) out.write("selected");%>>刷新</option>
								  			<option value="false" <%if(ValueStack.get("toolbar","freshOpener").equals("false")) out.write("selected");%>>不刷新</option>								  
										</select>	
                                        </td>										
										<td>
										<select name="closeWindow">
								       		<option value="">请选择</option>
								   			<option value="true" <%if(ValueStack.get("toolbar","closeWindow").equals("true")) out.write("selected");%>>关闭</option>
								   			<option value="false" <%if(ValueStack.get("toolbar","closeWindow").equals("false")) out.write("selected");%>>不关闭</option>								  
										</select>										
										</td>																     		
									</tr>
								  </s:iterator>
...
window.onload=function(){
	parent.iframe2.location.href="<%=contextPath%>/jsp/console/getFormToolbarBase.action?resourceName=<s:write property="resourceName"/>&pageKey=<s:write property="pageKey"/>&pageCode=<s:write property="pageCode"/>&toolbarKey=";
}
...
function addLine_f(){
  ...
  if(rows>=1){
     ...
     td.innerHTML="<input id=\"key\" name=\"key\"  type=\"hidden\" value=\"\"/><input id=\"id\" name=\"id\"  type=\"radio\" value=\"\" size=\"\" class=\"text\" onclick=\"javascript:loadToolbarBase('<%=contextPath%>/jsp/console/getFormToolbarBase.action?resourceName=<s:write property="resourceName"/>&pageKey=<s:write property="pageKey"/>&pageCode=<s:write property="pageCode"/>&toolbarKey=');\"/>";
     var td=tr.insertCell(1);     
     td.innerHTML="<input id=\"code\" name=\"code\"  type=\"text\" value=\"\" size=\"16\" class=\"text\"/>";
     var td=tr.insertCell(2);
     td.innerHTML="<input id=\"name\" name=\"name\"  type=\"text\" value=\"\" size=\"16\" class=\"text\"/>";    
     var td=tr.insertCell(3);     
     td.innerHTML="<select id=\"freshOpener\" name=\"freshOpener\">"+
                  "<option value=\"\">请选择</option>"+                
                  "<option value=\"true\">刷新</option>"+
                  "<option value=\"false\">不刷新</option>"+                 
                  "</select>";
    var td=tr.insertCell(4);     
     td.innerHTML="<select id=\"closeWindow\" name=\"closeWindow\">"+
                  "<option value=\"\">请选择</option>"+                
                  "<option value=\"true\">关闭</option>"+
                  "<option value=\"false\">不关闭</option>"+                 
                  "</select>";
  }
}
</script>
