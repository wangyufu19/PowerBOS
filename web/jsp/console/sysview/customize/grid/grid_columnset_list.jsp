	...
	<div class="box">		
		<div class="tabContent">
			...
			<div class="innerBox" id="tab1_content">			  
				<div class="innerBox1">							
					<div class="btnt">
						<button class="btn-able" onclick="javascript:doWith_f(submitF,'saveGridColumnSetList.action','您确定要保存吗?');">保存</button>&nbsp;&nbsp;
								...
								<tbody>
								  <s:iterator id="columnSet" name="columnSets" status="st">   								      
							        ...
							        >			
							            ...
							            <input id="id" name="id" type="radio" value="<s:write name="columnSet" property="key"/>" size="16" class="text" onclick="javascript:loadColumnSetAtt('<%=contextPath%>/jsp/console/getGridColumnSetAtt.action?resourceName=<s:write property="resourceName"/>&pageKey=<s:write property="pageKey"/>&pageCode=<s:write property="pageCode"/>&columnKey=<s:write name="columnSet" property="key"/>');"/>
							            </td> 				           
										<td><input id="name" name="name"  type="text" value="<s:property name="columnSet" value="name"/>" size="26" class="text"/></td>	
										<td><input id="chineseName" name="chineseName"  type="text" value="<s:property name="columnSet" value="chineseName"/>" size="26" class="text"/></td>	
										<td><input id="value" name="value"  type="text" value="<s:property name="columnSet" value="value"/>" size="26" class="text"/></td>																							     		
									</tr>
								  </s:iterator>
								</tbody>
							</table>	
...
window.onload=function(){
	parent.iframe2.location.href="<%=contextPath%>/jsp/console/getGridColumnSetAtt.action?resourceName=<s:write property="resourceName"/>&pageKey=<s:write property="pageKey"/>&pageCode=<s:write property="pageCode"/>&columnKey=";
}
...
function addLine_f(){
  ...
  if(rows>=1){
     ...
     td.innerHTML="<input id=\"key\" name=\"key\"  type=\"hidden\" value=\"\"/><input id=\"id\" name=\"id\"  type=\"radio\" value=\"\" size=\"\" class=\"text\" onclick=\"javascript:loadColumnSetAtt('<%=contextPath%>/jsp/console/getGridColumnSetAtt.action?resourceName=<s:write property="resourceName"/>&pageKey=<s:write property="pageKey"/>&pageCode=<s:write property="pageCode"/>&columnKey=');\"/>";
