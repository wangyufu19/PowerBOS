...
function ComboBox_build(arr)
{
    ...
    for(var i=0;i<arrlen;i++)
    {
		...
		{
			//this.choose(arr[i].value,arr[i].text);
			//alert(arr[i].value + ", " + arr[i].text);
			sel = "selected";
		}

        strs[i] = '<tr>' +
			'<td class="combo-item" onClick="'+this.name+'.choose(\''+arr[i].value+'\',\''+arr[i].text+'\');'+this.name+'.opslist.style.display=\'none\';"' +
			'onMouseOver="this.className=\'combo-hilite\';" onMouseOut="this.className=\'combo-item\'" '+ sel +' >&nbsp;'+arr[i].text+'&nbsp;</td>' +
			'</tr>';
		sel = "";
    }
...
function ComboBox()
{
	...
	this.setDefaultValue = ComboBox_default;
...
function ComboBoxItem(text,value,selected)
{
    this.text  = text;
    this.value = value;
