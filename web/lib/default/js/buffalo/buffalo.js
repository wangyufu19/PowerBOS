...
Buffalo.prototype = {
	...
	remoteCall: function(service, params, callback) {	
		...
		var newUrl = this.gateway+"/ajaxapp/ajaxapp/"+serviceId;
...
Buffalo.Default = {
	...
	showLoading : function(state) {
		...
		if (this.loadingPane == null) {
			...
			el.style.cssText="display:none;font-family:Verdana;font-size:15px;font-weight:bold;border:1px solid #00CC00;background-color:#A4FFA4;padding:1px;position:absolute; left:50%;top:200px;margin-left:-55px;width:110px; height:14px; z-index:10000";
			//el.innerHTML="loading... ";
			if(document.all)
				el.style.marginTop = (document.documentElement.scrollTop + 200) + "px";
			else
		...
		if (state) {
			...
			document.getElementById("buffalo_loading").style.display = "none";
			//document.getElementById("buffalo_loading").innerHTML = "";
			//document.getElementById("buffalo_loading").style.cssText = "";
			document.body.removeChild(document.getElementById("buffalo_loading"))
		}
	},
	...
	doSwitchPart: function(viewName) {
		...
		try {
			...
			if (/MSIE/.test(navigator.userAgent)) {
				var bfViewHackKey = "_bfviewhackkey_=" + (new Date());
			...
			switch (element.type) {
				...
				default : 
					if (ignoreButton) {
						...
						if(element.tagName=="OBJECT"||element.tagName=="object") {}
						else
						object[element.name] = element.value;
					}
					break;
...
formToBeanWithHiddenList : function(form, boClass, hiddenObjArr, ignoreButton) {
		var object = {};
		if (boClass) { object[Buffalo.BOCLASS] = boClass; } else{
			object[Buffalo.BOCLASS] = "java.util.Map";
		}
		...
		for (var i = 0; i < elements.length;i++) {
			...
			switch (element.type) {
				...
				default :
					var isMatch = false;
					for(var j = 0; j < hiddenObjArr.length; j++){
						if(element.name==hiddenObjArr[j]){
							isMatch = true;
							break;
						}
					}
					...
					//alert("len:"+Form.getInputs(form,"hidden",element.name).length);
					if (ignoreButton) {
						if (element.type != "submit" && element.type != "button" 
							&& element.type != "reset") {
							object[element.name] = element.value;
